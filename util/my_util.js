...
var MyModal = class {
     ...
     */
    static Info(content, title='Info', width='1000px', height='600px', id_str="") {
    ...
    static Alert(content, ok_fun = null, width=null, height=null, title='Info') {
     ...
     * @param {*} ext_btn_obj, 额外按钮信息，默认为null，格式为{ text: '按钮文本', fun: function(){...} }
     * @param {*} title 弹框标题
     */
    static Confirm(content, ok_fun=null, cancele_fun=null, ext_btn_obj=null, title='Confirm') {
        ...
        if(ext_btn_obj){
            ...
            $("#my-confirm-third").text(ext_btn_obj.text);
            $("#my-confirm-third").off("click").click(ext_btn_obj.fun);
        }else{
...
var MyUnit = class{
    static FileSizeStr(byte_size){
        let size_str = '';
        if(byte_size < 1024){
            size_str = byte_size + ' B';
        }else if(byte_size < 1024 * 1024){
            size_str = (byte_size / 1024).toFixed(1) + ' KB';
        }else if(byte_size < 1024 * 1024 * 1024){
            size_str = (byte_size / 1024 / 1024).toFixed(1) + ' MB';
        }else{
            size_str = (byte_size / 1024 / 1024 / 1024).toFixed(1) + ' GB';
        }
        return size_str;
    }
}
...
var MyJQueryUi = class {
	// 初始化自动补全组件
	static InitAotoCompleteStyle(height=160){
		$(".ui-autocomplete").css("max-height", height + "px").css("min-height", "60px").css("overflow-y", "auto").css('z-index', 200).css({'max-width': '1000px','overflow-x': 'hidden'});
	}

	/**
	 * 对input提供搜索匹配能力
	 * @param {*} dom_patter ，用于选择元素的标签
	 * @param {*} complete_data 用于选择的数据
	 * @param {*} select_fun 选择数据后的额外动作, 如果complete_data含有label则函数参数为 value,label , 否则参数只有value。如果除了填充值到文本框外无特殊操作，本参数无需设置
	 * @param int max_length 下拉框高度
     * 
     * 例子：通过ajax获取json数据并设置自动补全
     *  MyAjax.ASyncCall("url", function(data){
     *      MyJQueryUi.AutoComplete("#AddRecordVal", data);
     *  }, true);
	 */
	static AutoComplete(dom_patter, complete_data, select_fun = null, max_height=160){
		$(dom_patter).autocomplete({
			source: function(request, response) {
			   	// 构建正则表达式，支持正则匹配
			   	var pattern = new RegExp(request.term.replace(/\s+/g, '.*'), 'i');
			   	// 筛选匹配的列表项
			   	var matches = $.grep(complete_data, function(item) {
			...
			select: function( event, ui ) {
				...
				if(select_fun){
					if(typeof ui.item == 'string'){
						select_fun(ui.item.value);
					}else{
						select_fun(ui.item.value, ui.item.label);
					}
				}
			   	return false;
		   }
	   ...
	   MyJQueryUi.InitAotoCompleteStyle(max_height);
	}
}
